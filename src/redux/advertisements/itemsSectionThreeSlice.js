...
export const getItemsSectionThree = createAsyncThunk(
  ...
  async ({ token }, { rejectWithValue }) => {
    try {
      ...
      const response = await axios.get(`${baseUrl}/api/v1/items?isActive=true&inSectionThree=true&page=1&limit=25`, {
        timeout: 10000,
...
export const itemsSectionThreeSlice = createSlice({
  ...
  extraReducers: {
    ...
    [addItemToSectionThree.fulfilled]: (state, action) => {
      ...
      state.addItemToSectionThreeError = '';
      state.itemsSectionThree = [...state.itemsSectionThree, action.payload.data.item];
    },
