  ...
  async ({ token }, { rejectWithValue }) => {
    try {
      ...
      const response = await axios.get(
        `${baseUrl}/api/v1/users?type=company&isActive=true&inSectionOne=true&page=1&limit=25`,
        {
          timeout: 10000,
  ...
  async ({ token, id }, { rejectWithValue }) => {
    try {
      ...
      const response = await axios.post(
        `${baseUrl}/api/v1/users/inSectionOne/${id}`,
        {
          option: true,
        },
        {
...
export const companiesSectionOneSlice = createSlice({
  ...
  extraReducers: {
    ...
    [addCompanyToSectionOne.fulfilled]: (state, action) => {
      ...
      state.addCompanyToSectionOneError = '';
      state.companiesSectionOne = [...state.companiesSectionOne, action.payload.data.user];
    },
