...
export const getItemsSectionTwo = createAsyncThunk(
  ...
  async ({ token }, { rejectWithValue }) => {
    try {
      ...
      const response = await axios.get(`${baseUrl}/api/v1/items?isActive=true&inSectionTwo=true&page=1&limit=25`, {
        timeout: 10000,
...
export const itemsSectionTwoSlice = createSlice({
  ...
  extraReducers: {
    ...
    [addItemToSectionTwo.fulfilled]: (state, action) => {
      ...
      state.addItemToSectionTwoError = '';
      state.itemsSectionTwo = [...state.itemsSectionTwo, action.payload.data.item];
    },
    ...
    [removeItemFromSectionTwo.fulfilled]: (state, action) => {
      ...
      state.removeItemFromSectionTwoError = '';
      state.itemsSectionTwo = state.itemsSectionTwo.filter((company) => company._id !== action.payload.data.item._id);
    },
