...
export const getBaskets = createAsyncThunk('baskets/getBaskets', async ({ token }, { rejectWithValue, getState }) => {
  const {
    baskets: { pageState },
  } = getState();
...
export const addBasket = createAsyncThunk('baskets/addBasket', async ({ data, token }, { rejectWithValue }) => {
  try {
    CancelToken = axios.CancelToken;
    source = CancelToken.source();
...
export const updateBasket = createAsyncThunk('baskets/updateBasket', async ({ data, token }, { rejectWithValue }) => {
  try {
    CancelToken = axios.CancelToken;
    source = CancelToken.source();
...
export const basketsSlice = createSlice({
  ...
  reducers: {
    ...
    removeBasketFromState: (state, action) => {
      const updatedBaskets = state.baskets.filter((b) => b._id !== action.payload.basketId);
