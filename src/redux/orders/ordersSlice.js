...
import { BASEURL, DateOptions, OrdersStatusOptions, UserTypeConstants } from '../../utils/constants';
...
export const updateBasketOrder = createAsyncThunk(
  ...
  async ({ obj, id, token }, { rejectWithValue }) => {
    try {
      ...
      const response = await axios.post(`${BASEURL}/ordered-baskets/update?id=${id}`, obj, {
        cancelToken: source.token,
        headers: {
          Authorization: `Bearer ${token}`,
        },
...
export const ordersSlice = createSlice({
  ...
  extraReducers: {
    ...
    [getBasketsOrders.fulfilled]: (state, action) => {
      state.status = 'succeeded';
      state.basketOrders = [...state.basketOrders, ...action.payload.data.basketOrders];
    ...
    [deleteBasketOrder.fulfilled]: (state, action) => {
      state.status = 'succeeded';
      state.basketOrders = state.basketOrders.filter((o) => o._id !== action.payload.data.basketOrderId);
