        ...
        <View style={styles.details}>
          ...
          <View style={{ ...styles.row, ...styles.center }}>
            ...
            {order.status === OrdersStatusOptions.WILL_DONT_SERVER && (
              <Text style={styles.status}>{order.couldNotDeliverDate.split('T')[0]}</Text>
            )}
            {order.status === OrdersStatusOptions.CONFIRM && (
              <Text style={styles.status}>{order.confirmDate.split('T')[0]}</Text>
            )}
            {order.status === OrdersStatusOptions.DELIVERY && (
              <Text style={styles.status}>
                {order.deliverDate?.split('T')[0]}{' '}
                {order.deliverTime ? `---${t('time-label')}: ${order.deliverTime}` : ''}
              </Text>
            )}
            {order.status === OrdersStatusOptions.SHIPPING && (
              <Text style={styles.status}>
                {order.shippedDate ? order.shippedDate.split('T')[0] : t('shipped-done')}
                {order.shippedTime ? `---${t('time-label')}: ${order.shippedTime}` : ''}
              </Text>
            )}
          </View>
        </View>
